...
const firstParam = R.compose(R.head, params)
const isNotObj = R.compose(R.not, isObj)
const paramsLength = R.compose(R.length, params)
const isUnary = R.compose(equalsOne, paramsLength)

const betaReduce = R.curry((f, g) => {
  if (isUnary(f)) {
    return maybeUnnest(deepReplace(firstParam(f), g, body(f)))
  }
  if (!recursiveContains(firstParam(f), body(f))) return decrementBody(reduceParams(f))
  if (isNotObj(g)) {
